...
├── tracking/               # 追踪模块
│   ├── geofencing.py      # 电子围栏
│   ├── eta_calculator.py  # ETA 预估
│   ├── position_tracker.py # 位置追踪
│   └── gps_simulator.py   # GPS 模拟器
│
├── models/                 # 数据模型层
│   ├── customer.py        # 客户模型
...
calculator = CostCalculator(customers, vehicle_config, params)
result = calculator.calculate(solution)
...
```python
from tracking.position_tracker import PositionTracker, VehicleStatus
from tracking.gps_simulator import GPSSimulator, SimulationConfig

# 初始化追踪器
tracker = PositionTracker(solution, vehicle_config)

# 更新车辆位置
tracker.update_position(vehicle_id=0, lat=39.9142, lon=116.4174, speed=45.0)

# 更新车辆状态
tracker.update_status(vehicle_id=0, status=VehicleStatus.SERVING)

# 获取活跃车辆
active_vehicles = tracker.get_active_vehicles()

# 获取延误车辆
delayed_vehicles = tracker.get_delayed_vehicles()

# 导出轨迹 GeoJSON
geojson = tracker.export_trajectory_geojson(vehicle_id=0)

# 启动 GPS 模拟
config = SimulationConfig(update_interval=5.0, simulate_traffic=True)
simulator = GPSSimulator(solution, vehicle_config, tracker, config)
simulator.start()
```
...
| `/api/v1/jobs/{job_id}/result` | GET GET | 获取求解结果 |
...
| tracking | test_geofencing.py, test_eta_calculator.py, test_position_tracker.py, test_gps_simulator.py | ~100 |
| api | test_api_solvers.py, test_scenarios.py | ~50 |
